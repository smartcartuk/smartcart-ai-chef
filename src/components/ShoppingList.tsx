...
export const ShoppingList: React.FC<ShoppingListProps> = ({ 
  ...
  const ingredients = React.useMemo(() => {
    const allIngredients: Array<{ name: string; amount: string; store: string; price: number }> = [];
  ...
  const stores = [
    ...
    { 
      ...
      savings: totalWeeklyCosts ? Math.max(0, (totalWeeklyCosts.sainsburys - totalWeeklyCosts.tesco)) : 4.45,
    ...
    { 
      ...
      savings: totalWeeklyCosts ? Math.max(0, (totalWeeklyCosts.tesco - totalWeeklyCosts.asda)) : 3.65,
    ...
    { 
      ...
      savings: totalWeeklyCosts ? Math.max(0, (totalWeeklyCosts.tesco - totalWeeklyCosts.aldi)) : 7.90,
  ...
  const generateOptimizedRoute = () => {
    ...
    const categories = {
      'Fresh Produce': ['tomatoes', 'onions', 'lettuce', 'carrots', 'potatoes'],
      'Meat & Fish': ['chicken', 'beef', 'salmon', 'prawns'],
      'Dairy': ['milk', 'cheese', 'yogurt', 'butter'],
      'Pantry': ['rice', 'pasta', 'oil', 'spices', 'flour']
    };
        ...
        <TabsList className="grid w-full grid-cols-4 bg-white border shadow-sm">
          {stores.map(store => (
            <TabsTrigger 
              key={store.id}
              value={store.id}
              className="flex flex-col items-center space-y-1 p-4 data-[state=active]:bg-blue-50 data-[state=active]:border-blue-200"
            >
                            ...
                            <div className="flex items-center space-x-3">
                              ...
                              <div>
                                <div className={`font-medium ${
                                  checkedItems.has(item.name) ? 'line-through text-gray-500' : 'text-gray-900'
                                }`}>
                                  {item.name}
                                </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {stores.map(store => (
              <div key={store.id} className="text-center p-4 bg-white rounded-lg shadow-sm">
                <div className="text-2xl mb-2">{store.logo}</div>
                <div className="font-medium text-sm">{store.name}</div>
